...
export const getSubscriptionPlans = async () => {
  try {
    ...
    // Procesar los features para convertir objeto JSONB a array
    const processedData = data.map(plan => ({
      ...plan,
      features: processPlanFeatures(plan.features)
    }));

    console.log(`✅ Encontrados ${processedData.length} planes`);
    console.log('🔍 Planes procesados:', processedData);
...
/**
 * Procesar features de un plan - convertir objeto JSONB a array
 */
const processPlanFeatures = (features) => {
  try {
    // Si features es null o undefined, retornar array vacío
    if (!features) {
      console.log('⚠️ Features is null/undefined, returning empty array');
      return [];
    }
    ...
    // Si es un objeto, convertir los valores a array
    if (typeof features === 'object') {
      const featuresArray = Object.values(features);
      console.log('🔄 Converted features object to array:', featuresArray);
    ...
    if (typeof features === 'string') {
      try {
        ...
        console.warn('⚠️ Could not parse features string:', features);
      }
    }

    console.warn('⚠️ Unknown features format, returning empty array:', features);
    return [];
  } catch (error) {
    console.error('❌ Error processing plan features:', error);
    return [];
  }
};
...
export const getSubscriptionPlan = async (planId) => {
  try {
    ...
    // Procesar features del plan individual
    const processedPlan = {
      ...data,
      features: processPlanFeatures(data.features)
    };

    console.log('✅ Plan obtenido y procesado:', processedPlan);
