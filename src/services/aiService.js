...
import { logInfo, logError, LOG_CATEGORIES } from './logger';
  ...
  async adaptRecipeWithAI(recipe, userPreferences) {
    try {
      if (!this.preferredService) {
        const errorMsg = 'No hay servicios de IA configurados';
        logError(LOG_CATEGORIES.API, errorMsg, new Error(errorMsg));
        throw new Error(errorMsg);
      }

      logInfo(LOG_CATEGORIES.API, `Iniciando adaptación de receta: ${recipe.title}`, {
      ...
      if (this.preferredService === 'CLAUDE') {
        result = await this.adaptWithClaude(prompt);
      } else if (this.preferredService === 'OPENAI') {
        result = await this.adaptWithOpenAI(prompt);
      }

      logInfo(LOG_CATEGORIES.API, 'Adaptación completada exitosamente', {
        service: this.preferredService,
        recipeTitle: recipe.title
      });
      ...
      logError(LOG_CATEGORIES.API, 'Error adaptando receta con IA', {
        error,
...
Responde en formato JSON con la siguiente estructura:
{
  ...
  async adaptWithOpenAI(prompt) {
    console.log('═══════════════════════════════════════════════════════');
    console.log('🤖🤖🤖 ADAPTANDO RECETA CON OPENAI 🤖🤖🤖');
    console.log('═══════════════════════════════════════════════════════');

    const config = getApiConfig('OPENAI');

    console.log('🔑 OpenAI API Key disponible:', config.API_KEY ? `${config.API_KEY.substring(0, 10)}...` : 'NO DISPONIBLE');
    console.log('🔑 OpenAI API Key length:', config.API_KEY?.length);
    console.log('🔑 OpenAI API Key full:', config.API_KEY || 'NO DISPONIBLE');
    console.log('═══════════════════════════════════════════════════════');

    // Log en Debug Logs
    logInfo(LOG_CATEGORIES.API, 'Iniciando adaptación de receta con OpenAI', {
      apiKeyPresent: !!config.API_KEY && config.API_KEY !== 'TU_API_KEY_AQUI',
      apiKeyLength: config.API_KEY?.length,
      apiKeyPrefix: config.API_KEY ? config.API_KEY.substring(0, 10) : 'N/A',
      model: config.MODEL,
